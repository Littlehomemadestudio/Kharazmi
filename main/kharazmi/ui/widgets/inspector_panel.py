...
class InspectorPanel(QScrollArea):
    ...
    def _build_header(self) -> None:
        ...
        self._task_title_label = QLabel("وظیفه‌ای انتخاب نشده")
    ...
    def _build_basic_group(self) -> None:
        ...
        layout.addRow("ریسک", self._risk_combo)

        # Tags
        self._tags_edit = QLineEdit()
        self._tags_edit.setPlaceholderText("با کاما جدا کنید")
        self._tags_edit.editingFinished.connect(self._on_tags_changed)
        layout.addRow("برچسب‌ها", self._tags_edit)

        self._layout.addWidget(group)

    # ساخت گروه زمان‌بندی
    def _build_schedule_group(self) -> None:
        group = QGroupBox("زمان‌بندی (محاسبه‌شده)")
        layout = QFormLayout(group)
        layout.setSpacing(4)
        ...
        layout.addRow("شروع زودهنگام", self._lbl_es)
        layout.addRow("پایان زودهنگام", self._lbl_ef)
        layout.addRow("شروع دیرهنگام", self._lbl_ls)
        layout.addRow("پایان دیرهنگام", self._lbl_lf)
        layout.addRow("وقفه کل", self._lbl_slack)
        layout.addRow("وضعیت", self._lbl_critical)
        ...
        self._es_constraint.setSpecialValueText("(هیچ)")
        self._es_constraint.dateTimeChanged.connect(self._on_es_constraint_changed)
        layout.addRow("شروع زودهنگام", self._es_constraint)
    ...
    def _build_pert_group(self) -> None:
        group = QGroupBox("برآورد سه‌نقطه‌ای پرت")
        layout = QFormLayout(group)
        ...
        layout.addRow("خوش‌بینانه", self._pert_o)
        layout.addRow("محتمل‌ترین", self._pert_m)
        layout.addRow("بدبینانه", self._pert_p)
        layout.addRow("واحد", self._pert_unit)
        ...
        layout.addRow("مورد انتظار", self._pert_expected)

        self._clear_pert_btn = QPushButton("پاک کردن پرت")
    ...
    def load_task(self, task: Optional[Task]) -> None:
        ...
        try:
            if task is None:
                self._task_title_label.setText("وظیفه‌ای انتخاب نشده")
                self._task_title_label.setStyleSheet(
            ...
            if task.slack:
                ...
                self._lbl_critical.setText("بحرانی" if task.is_critical else "غیربحرانی")
            ...
            self._transitions_label.setText(
                f"انتقال‌های مجاز: {', '.join(s.value for s in legal) or '(پایانی)'}"
            )
    ...
    def _on_pert_changed(self) -> None:
        ...
        try:
            ...
            if not (opt.minutes <= ml.minutes <= pess.minutes):
                self._pert_expected.setText("نامعتبر: O≤M≤P مورد نیاز است")
                return
            ...
            self._pert_expected.setText(f"خطا: {e}")
    ...
    def _quick_change_status(self, new_status: TaskStatus) -> None:
        ...
        try:
            ...
            QMessageBox.warning(self, "انتقال غیرمجاز", str(e))
