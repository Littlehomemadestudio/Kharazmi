...
def get_persian_date(date_obj):
    ...
    try:
        # This is a simple approximation - for production, use jdatetime library
        persian_months = PERSIAN_MONTHS
        persian_weekdays = PERSIAN_WEEKDAYS
        
        # Simple conversion (this is approximate)
        weekday = date_obj.weekday()
        persian_weekday = persian_weekdays[(weekday + 1) % 7]  # Adjust for Persian week starting Saturday
        
        month = date_obj.month - 1
        persian_month = persian_months[month % 12]
        
        return f"{persian_weekday}، {date_obj.day} {persian_month} {date_obj.year}"
    except:
...
TRANSLATIONS = {
    ...
    "fa": {
        ...
        "task_completed": "وظيفه تکميل شد!",
        "confirm_delete": "آيا مطمئن هستيد که مي خواهيد اين وظيفه را حذف کنيد؟",
        "yes": "بله",
        "no": "خير",
        "cancel": "لغو",
        "save": "ذخيره",
        "task_title": "عنوان وظيفه",
        "task_time": "زمان",
        "task_duration": "مدت زمان",
        "task_priority": "اولويت",
        "task_category": "دسته بندي",
        "high": "بالا",
        "medium": "متوسط",
        "low": "پايين",
        "academic": "آکادميک",
        ...
        "chore": "کار خانه",
        "social": "اجتماعي",
        "communication": "ارتباطي",
        "domestic": "خانگي",
        "professional": "حرفه اي",
        "analytical": "تحليلي",
        "other": "ساير",
        "morning": "صبح",
        ...
        "auto_save": "ذخيره خودکار",
        "compact_mode": "حالت فشرده"
    }
}

# Persian calendar helpers
PERSIAN_MONTHS = ["فروردين", "ارديبهشت", "خرداد", "تير", "مرداد", "شهريور", "مهر", "آبان", "آذر", "دي", "بهمن", "اسفند"]
PERSIAN_WEEKDAYS = ["شنبه", "يکشنبه", "دوشنبه", "سه شنبه", "چهارشنبه", "پنج شنبه", "جمعه"]
...
class PlannerUI:
    ...
    def _create_header(self, parent):
        ...
        # Get date string based on language
        if self.current_language == 'fa':
            # Persian date display
            date_str = get_persian_date(self.selected_date)
        else:
            date_str = self.selected_date.strftime("%A, %B %d, %Y")
            
        # Set text anchor based on language
        anchor = 'e' if self.rtl_mode else 'w'
        
        self.header_label = ctk.CTkLabel(
            header_frame, text=date_str,
            font=ctk.CTkFont(family="SF Pro Display", size=36, weight="bold"),
            text_color=self.colors['text_primary'], anchor=anchor
        )
        self.header_label.pack(side='left' if not self.rtl_mode else 'right', fill='x', expand=True)
        
        tagline = ctk.CTkLabel(
            header_frame, text=self.t("tagline"),
            font=ctk.CTkFont(family="Vazirmatn" if self.rtl_mode else "SF Pro Text", size=14),
            text_color=self.colors['text_tertiary'], anchor=anchor
        )
        tagline.pack(side='left' if not self.rtl_mode else 'right', padx=(20, 0) if not self.rtl_mode else (0, 20))
    ...
    def _create_tasks_panel(self, parent):
        ...
        tasks_title = ctk.CTkLabel(
            ...
            font=ctk.CTkFont(family="Vazirmatn" if self.rtl_mode else "SF Pro Display", size=22, weight="bold"),
            text_color=self.colors['text_primary'], anchor='e' if self.rtl_mode else 'w'
        )
        tasks_title.pack(side='right' if self.rtl_mode else 'left', fill='x', expand=True)
        
        self.task_count_label = ctk.CTkLabel(
            tasks_header, text=self.t("task_count"),
            font=ctk.CTkFont(family="Vazirmatn" if self.rtl_mode else "SF Pro Text", size=14, weight="bold"),
    ...
    def _create_action_bar(self, parent):
        ...
        left_buttons.pack(side='right' if self.rtl_mode else 'left', fill='y')
        
        add_btn = self._create_primary_button(left_buttons, self.t("add_task"), self.on_add_task)
        add_btn.pack(side='left', padx=(0, 10))
        
        self.edit_btn = self._create_secondary_button(left_buttons, self.t("edit"), self._handle_edit_task)
        self.edit_btn.pack(side='left', padx=(0, 10))
        self.edit_btn.configure(state='disabled')
        
        self.delete_btn = self._create_secondary_button(
            left_buttons, self.t("delete"), self._handle_delete_task,
        ...
        right_frame.pack(side='left' if self.rtl_mode else 'right', fill='y')
        ...
        # Map translated names back to English for internal use
        view_mapping = [
            (self.t("month"), "Month"),
            (self.t("week"), "Week"), 
            (self.t("day"), "Day")
        ...
        for translated_name, internal_name in view_mapping:
            btn = ctk.CTkButton(
                view_button_frame,
                text=translated_name,
                command=lambda v=internal_name: self._change_view(v),
                font=ctk.CTkFont(family="Vazirmatn" if self.rtl_mode else "SF Pro Text", size=14, weight="bold"),
                corner_radius=12,
                height=35,
                width=80
            ...
            self.view_buttons[internal_name] = btn
        ...
        ai_btn = self._create_ai_button(right_frame, self.t("ai_assistant"), self._toggle_ai_window)
        ai_btn.pack(side='left', padx=(0, 10))
        
        settings_btn = ctk.CTkButton(
            right_frame, text=self.t("settings"), width=140, height=50,
            font=ctk.CTkFont(family="Vazirmatn" if self.rtl_mode else "SF Pro Text", size=15, weight="bold"),
            fg_color=self.colors['bg_secondary'],
    ...
    def _show_ai_window(self):
        ...
        self.ai_window.title("🤖 AI Assistant")
        ...
        if not self.ai_assistant.is_loaded and not self.ai_assistant.loading:
            self._add_ai_message(self.t("connecting") + "\n\n" + self.t("ensure_ollama"), "system")
            self.ai_assistant.load_model(self._on_model_loaded)
        else:
            welcome_msg = self.t("welcome")
    ...
    def _create_ai_window_content(self):
        ...
        ctk.CTkLabel(
            header, text=self.t("ai_assistant"),
            font=ctk.CTkFont(family="Vazirmatn" if self.rtl_mode else "SF Pro Display", size=24, weight="bold"),
            text_color=self.colors['text_primary']
        ).pack(side='right' if self.rtl_mode else 'left')
        
        close_btn = ctk.CTkButton(
            header, text=self.t("close"), width=40, height=40,
        ...
        close_btn.pack(side='left' if self.rtl_mode else 'right')
        ...
        self.ai_input.pack(fill='both', padx=(10, 0) if self.rtl_mode else (0, 10), side='right' if self.rtl_mode else 'left', expand=True)
        
        send_btn = ctk.CTkButton(
            input_frame, text=self.t("send"), width=60, height=80,
        ...
        send_btn.pack(side='left' if self.rtl_mode else 'right')
        
        # Reset conversation button
        reset_btn = ctk.CTkButton(
            self.ai_window, text=self.t("new_conversation"),
            font=ctk.CTkFont(family="Vazirmatn" if self.rtl_mode else "SF Pro Text", size=13),
            fg_color=self.colors['bg_hover'],
            hover_color=self.colors['border'],
            text_color=self.colors['text_secondary'],
            height=35,
    ...
    def _send_ai_message(self):
        ...
        if intent_info['intent'] == 'planning':
            ...
            self._add_ai_message(self.t("thinking"), "system")
    ...
    def _start_personalization_flow(self, user_input: str):
        ...
        self._add_ai_message("🤔 Analyzing your message and extracting tasks...", "system")
    ...
    def _ask_next_personal_question(self):
        ...
        self._add_ai_message(self.t("thinking") + " - " + "Generating personalized question...", "system")
    ...
    def _reset_ai_conversation(self):
        ...
        reset_msg = "🔄 Conversation reset. I'm ready to help with planning tasks or just chat! What would you like to do?"
    ...
    def _create_settings_panel_content(self):
        ...
        ctk.CTkLabel(
            notif_section, text="🔔 Notifications",
        ...
        ctk.CTkLabel(
            reminder_frame, text="Remind me before (minutes):",
        ...
        ctk.CTkLabel(
            interval_frame, text="Save interval (seconds):",
            font=ctk.CTkFont(family="SF Pro Text", size=13),
            text_color=self.colors['text_secondary']
        ).pack(side='left', padx=(0, 10))
        ...
        ctk.CTkLabel(
            display_section, text="🖥️ Display",
            font=ctk.CTkFont(family="SF Pro Display", size=18, weight="bold"),
            text_color=self.colors['text_primary']
        ).pack(anchor='w', padx=20, pady=(20, 15))
        ...
        ctk.CTkLabel(
            startup_frame, text="Startup View:",
            font=ctk.CTkFont(family="SF Pro Text", size=13),
            text_color=self.colors['text_secondary']
        ).pack(side='left')
        
        startup_var = ctk.StringVar(value=self.settings['startup_view'])
        startup_menu = ctk.CTkOptionMenu(
            startup_frame,
            values=['Month', 'Week', 'Day'],
            variable=startup_var,
            command=lambda v: self._update_setting('startup_view', v),
            width=120,
            fg_color=self.colors['bg_hover'],
        ...
        ctk.CTkLabel(
            general_section, text="⚙️ General",
            font=ctk.CTkFont(family="SF Pro Display", size=18, weight="bold"),
            text_color=self.colors['text_primary']
        ).pack(anchor='w', padx=20, pady=(20, 15))
        ...
        ctk.CTkLabel(
            duration_frame, text="Default Duration:",
            font=ctk.CTkFont(family="SF Pro Text", size=13),
            text_color=self.colors['text_secondary']
        ).pack(side='left')
    ...
    def _create_day_button(self, parent, day, date_obj, is_today, is_selected, has_tasks):
        ...
        btn = ctk.CTkButton(
            ...
            font=ctk.CTkFont(family="SF Pro Text", size=15, weight="bold" if is_selected else "normal"),
    ...
    def _create_primary_button(self, parent, text, command):
        return ctk.CTkButton(parent, text=text, font=ctk.CTkFont(family="SF Pro Text", size=15, weight="bold"), fg_color=self.colors['accent_primary'], hover_color=self.colors['accent_secondary'], text_color='#FFFFFF', corner_radius=14, height=50, width=160, command=command)

    def _create_secondary_button(self, parent, text, command, fg_color=None, text_color=None, hover_color=None):
        return ctk.CTkButton(
            parent, text=text, font=ctk.CTkFont(family="SF Pro Text", size=15), 
    ...
    def _create_ai_button(self, parent, text, command):
        return ctk.CTkButton(parent, text=text, font=ctk.CTkFont(family="SF Pro Text", size=15, weight="bold"), fg_color=self.colors['accent_tertiary'], hover_color=self.colors['accent_secondary'], text_color='#FFFFFF', corner_radius=14, height=50, width=180, command=command)
    ...
    def _open_edit_dialog(self, task_data):
        ...
        today_btn = ctk.CTkButton(
            date_frame, text="Today", width=80, height=45,
            font=ctk.CTkFont(size=13),
            fg_color=self.colors['bg_hover'],
            hover_color=self.colors['border'],
            text_color=self.colors['text_primary'],
        ...
        # Tags field
        ctk.CTkLabel(
            content_scroll, text="Tags (comma-separated)",
            font=ctk.CTkFont(family="SF Pro Text", size=14, weight="bold"),
            text_color=self.colors['text_primary'], anchor='w'
        ).pack(fill='x', pady=(0, 8))
    ...
    def _refresh_tasks(self):
        # Don't refresh if UI is not fully initialized
        ...
        if not tasks:
            empty_text = self.t("no_tasks") + "\n" + self.t("add_task")
            empty_label = ctk.CTkLabel(self.tasks_scroll, text=empty_text, font=ctk.CTkFont(family="Vazirmatn" if self.rtl_mode else "SF Pro Text", size=15), text_color=self.colors['text_tertiary'], justify='center')
